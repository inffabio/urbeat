...
- Deploy source hardening: `04-deploy-application.ps1` and `scripts/deploy-internal.ps1` resolve the git HEAD/branch/status before uploading, block a dirty working tree by default, and accept optional `-AllowDirty` (ship an explicit local snapshot) and `-ExpectedCommit <sha>` (require HEAD to match). `deploy-all.ps1` exposes both and forwards them to the `application` step only. Both deploy paths write a `deployment-manifest.json` (commit/branch/dirty/generatedAtUtc/archive SHA256, never file contents or secrets) and upload it next to the deploy. Prefer passing `-ExpectedCommit` for production.
- Before any OCI SSH/SCP connection, send the port-knocking sequence `1230/tcp`, `5688/udp`, `9112/tcp` in that order using `scripts/criarDeployOracleCloud/port-knock.ps1`.
- `scripts/deploy-internal.ps1` targets `192.168.1.15`; it validates the deploy source before any remote command (including the remote `mkdir`), requires and uploads a `deployment-manifest.json` representing the exact commit and tarball sent, never rewrites `frontend/package.json` or `landing-page.component.ts`, and never commits or pushes (Git is not altered by the deploy). It is not the OCI deployment path.
