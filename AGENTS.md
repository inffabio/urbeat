...
- `scripts/deploy-internal.ps1` targets `192.168.1.15` and increments `frontend/package.json`; it validates the deploy source before packaging, writes/upload `deployment-manifest.json`, and never commits or pushes (Git is not altered by the deploy). It is not the OCI deployment path.
