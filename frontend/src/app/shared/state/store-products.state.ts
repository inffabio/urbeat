import { Directive, signal, computed, effect, inject, OnInit, ViewChild, ElementRef, Injector } from '@angular/core';
...
import { SubscriptionBannerStatus } from '../components/subscription-banner/subscription-banner.component';
import { StoreService } from '../../core/services/store.service';
import { SubscriptionService } from '../../core/services/subscription.service';
import { ToastService } from '../../core/services/toast.service';
import { Product, ProductCategory, ProductOptionGroup, ProductOptionGroupTemplate, ProductOptionItem, ProductVariation, ProductSaleMode, ProductWeightConfig } from '../models/product.model';
import { IMAGE_ACCEPT_ATTRIBUTE, IMAGE_FORMAT_ERROR, PRODUCT_IMAGE_MAX_BYTES, imageSizeError, isAllowedImageFile } from '../utils/image-upload.utils';
...
@Directive()
export abstract class StoreProductsState implements OnInit {
  ...
  protected validateProductsForAdvance(): boolean {
    const prods = this.products();
    if (prods.length === 0) { this.toast.showError('Cadastre pelo menos um produto com preço e imagem antes de avançar.'); return false; }
    for (const product of prods) {
      if (!product.price || product.price <= 0) {
        const sm = product.saleMode as string;
        if (sm === 'size' || sm === 'fixed_weight' || sm === 'variable_weight') continue;
        this.toast.showError(`O produto "${product.name}" precisa de um preço válido maior que zero.`); return false;
      }
      if (!product.imageUrl) { this.toast.showError(`O produto "${product.name}" precisa de uma imagem.`); return false; }
    }
    return true;
  }
